...
namespace csg_tree_generator
{
    public class Scene
    {
        ...
        public void AdjustTree(Node node, int treeHeight, int nodeHeight = 0)
        {
            ...
            {
                AdjustTree(node.left, treeHeight, nodeHeight + 1);
                AdjustTree(node.right, treeHeight, nodeHeight + 1);
                return;
            }

            int gapBetweenNodes = (int)Math.Pow(2, treeHeight - nodeHeight) * Draw.distanceHorizontal;
            if (node == node.parent.left)
            {
                node.X = node.parent.X - gapBetweenNodes / 2;
        ...
        public void ImportTree(string file_name)
        {
            ...
            AdjustTree(root, CountTreeHeight(root));
